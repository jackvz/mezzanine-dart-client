...
class OrdersApi {
  ...
  ///
  /// Destroy
  Future ordersDelete(int id) async {
    ...
    // create path and map variables
    String path = "/orders/{id}/".replaceAll("{format}","json").replaceAll("{" + "id" + "}", id.toString());
  ...
  Future<List<Order>> ordersList() async {
    ...
    if(response.statusCode >= 400) {
      ...
      return 
        (apiClient.deserialize(response.body, 'List<Order>') as List).map((item) => item as Order).toList();
    } else {
      return null;
    }
  }
  ...
  ///
  /// Partial update
  Future<Order> ordersPartialUpdate(int id, Order data) async {
    Object postBody = data;
  ...
  ///
  /// Retrieve
  Future<Order> ordersRead(int id) async {
    Object postBody = null;
