...
class GroupsApi {
  ...
  Future<Group> groupsCreate(Group data) async {
    ...
    if(response.statusCode >= 400) {
      ...
      return 
          apiClient.deserialize(response.body, 'Group') as Group ;
    } else {
      return null;
    }
  }
  ...
  Future groupsDelete(int id) async {
    ...
    // create path and map variables
    String path = "/groups/{id}/".replaceAll("{format}","json").replaceAll("{" + "id" + "}", id.toString());
  ...
  ///
  /// List all
  Future<List<Group>> groupsList() async {
    Object postBody = null;
    ...
    if(response.statusCode >= 400) {
      ...
      return 
        (apiClient.deserialize(response.body, 'List<Group>') as List).map((item) => item as Group).toList();
    } else {
      return null;
    }
  }
