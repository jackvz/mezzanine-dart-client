...
class CategoriesApi {
  ...
  ///
  /// Destroy
  Future categoriesDelete(int id) async {
  ...
  Future<List<Category>> categoriesList() async {
    ...
    if(response.statusCode >= 400) {
      ...
      return 
        (apiClient.deserialize(response.body, 'List<Category>') as List).map((item) => item as Category).toList();
    } else {
      return null;
    }
  }
  ...
  ///
  /// Partial update
  Future<Category> categoriesPartialUpdate(int id, Category data) async {
    Object postBody = data;
